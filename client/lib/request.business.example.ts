 ...
 * 标准业务接口使用示例
 */
import { request, createRequest, BusinessApiResponse, RequestError } from './request';
...
async function updateUser(userId: string, userData: Partial<User>) {
  try {
    const updatedUser = await request.businessPut<User>(`/api/users/${userId}`, userData);
...
async function getUserListWithFullResponse(params: UserListParams = {}) {
  try {
    const response = await request.get<BusinessApiResponse<UserListData>>('/api/users', params);
...
class UserService {
  ...
  async uploadAvatar(userId: string, file: File) {
    ...
    return this.apiClient.businessPost<{ avatarUrl: string }>(`/users/${userId}/avatar`, formData);
  }
}
...
class ProductService {
  ...
  async searchProducts(params: ProductSearchParams) {
    return this.apiClient.businessGet<ApiListResponse<Product>>('/products/search', params);
  }
...
async function productExample() {
  ...
  try {
    ...
    if (searchResult.list.length > 0) {
      const product = await productService.getProductById(searchResult.list[0].id);
