...
const generateMockUsers = (count: number): MockUser[] => {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const registrationDate = new Date(now.getTime() - randomDays * 24 * 60 * 60 * 1000);
    ...
    return {
      ...
      firstPurchaseTime: new Date(registrationDate.getTime() + Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(),
      lastActiveTime: new Date(now.getTime() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
