...
export class RuleConflictDetector {
  ...
  private detectActionConflicts(
    ...
    for (const existingRule of existingRules) {
      ...
      if (overlapScore > 0.3) {
        ...
        if (
          newRule.responseAction.actionType ===
            existingRule.responseAction.actionType &&
  ...
  private detectPriorityIssues(
    ...
    if (samePriorityRules.length > 0) {
      conflicts.push({
        ...
        conflictingRules: [
          newRule.ruleId,
          ...samePriorityRules.map((r) => r.ruleId),
        ],
  ...
  private detectPerformanceIssues(
    ...
    // 检查复杂条件
    const totalConditions = this.countTotalConditions(
      newRule.triggerConditions,
    );
   ...
   * 计算条件重叠度
   */
  private calculateConditionOverlap(
    conditions1: TriggerConditions,
    conditions2: TriggerConditions,
  ): number {
    ...
    const totalConditions = Math.max(
      allConditions1.length,
      allConditions2.length,
    );
  ...
  private isRuleDuplicate(rule1: OverrideRule, rule2: OverrideRule): boolean {
    return (
      this.calculateConditionOverlap(
        rule1.triggerConditions,
        rule2.triggerConditions,
      ) > 0.95 &&
      JSON.stringify(rule1.responseAction) ===
        JSON.stringify(rule2.responseAction)
    );
  }
  ...
  private countTotalConditions(conditions: TriggerConditions): number {
    return (
      conditions.eventConditions.length +
