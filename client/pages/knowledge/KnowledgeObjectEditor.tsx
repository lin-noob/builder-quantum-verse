...
const KnowledgeObjectEditor: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    console.log("Saving Knowledge Object:", formData);
    alert("对象保存成功 (模拟)");
    navigate(-1);
  };
  ...
  const updateAction = (index: number, field: string, value: any) => {
    ...
    if (field === 'conditions') {
       (newActions[index] as any)[field] = typeof value === 'string' ? value.split(',').map(s => s.trim()).filter(Boolean) : value;
    } else {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {isEditing ? `ID: ${formData.id}` : '定义新的业务实体模型'}
            </p>
          </div>
        </div>
                        ...
                        <div className="grid grid-cols-12 gap-8">
                           <div className="col-span-8 space-y-6">
                              <div className="grid grid-cols-2 gap-6">
                                 <div className="space-y-2">
                                  <Label className="text-sm font-medium text-slate-700">对象名称 <span className="text-red-500">*</span></Label>
                                  <Input 
                                    value={formData.name || ""} 
                                ...
                                <div className="space-y-2">
                                  <Label className="text-sm font-medium text-slate-700">唯一标识 (ID) <span className="text-red-500">*</span></Label>
                                  <Input 
                                    value={formData.id || ""} 
                           ...
                           <div className="col-span-4 bg-slate-50 rounded-lg p-5 border border-slate-100">
                              ...
                              <div className="space-y-3">
                                {[
                                  { id: "Master", name: "主数据 (Master)", icon: Database, desc: "核心业务实体，如客户、产品。" },
                                  { id: "Transaction", name: "交易数据 (Transaction)", icon: ShoppingCart, desc: "业务过程记录，如订单、流水。" },
                                  { id: "Result", name: "结果数据 (Result)", icon: FileText, desc: "分析或计算产生的衍生数据。" },
                                ].map((type) => (
                                  <div
                                    key={type.id}
                                    ...
                                    `}
                                  >
                                    <div className={`mt-0.5 p-1.5 rounded-md ${formData.type === type.id ? 'bg-blue-200 text-blue-700' : 'bg-slate-100 text-slate-500'}`}>
                                      <type.icon className="h-4 w-4" />
                                    </div>
                                    <div>
                                      <div className={`text-sm font-bold ${formData.type === type.id ? 'text-blue-800' : 'text-slate-700'}`}>{type.name}</div>
                            ...
                            {(formData.actions || []).map((action, index) => (
                              <TableRow key={index}>
                                ...
                                <TableCell>
                                  ...
                                  >
                                    <SelectTrigger className={`h-8 ${action.riskLevel === 'High' ? 'text-red-600 font-medium' : ''}`}>
                                      <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                ...
                                <TableCell>
                                  <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 hover:text-red-600" onClick={() => removeAction(index)}>
                                    <Trash2 className="h-3 w-3" />
                                  </Button>
                                </TableCell>
                              </TableRow>
                          ...
                          <TableBody>
                            {(formData.rules || []).map((rule, index) => (
                              <TableRow key={index}>
                                ...
                                <TableCell className="align-top">
                                  <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 hover:text-red-600" onClick={() => removeRule(index)}>
                                    <Trash2 className="h-3 w-3" />
                                  </Button>
                                </TableCell>
                              </TableRow>
