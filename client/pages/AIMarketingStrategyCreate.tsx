  ...
  const commonPurposeOptions = [
    {
      ...
      content: '当识别到用户在购物车页面停留时间超过3分钟且未完成支付，或者用户尝试离开结账页面时，系统将自动展示个性化挽留弹窗。结合用户的浏览历史和购买偏好，智能推荐相似产品或提供限时优惠券（如"专属8折优惠，仅限10分钟"），同时展示该商品的稀缺性提示（如"仅剩3件"）和��会证明（如"已有1280人购买此商品"）。针对高价值用户，可额外提供免费配送或延长退换货期限等增值服务，通过多重激励机制最大化转化可能性，降低购物车放弃率。'
    },
    {
      title: '欢迎新用户，并根据其兴趣进行初步引导',
      content: '当系统���测到新用户首次访问网站或应用时，基于用户的注册信息、访问来源渠道、浏览设备类型等数据，智能展示个性���欢迎引导流程。通过分析用户点击的商品类别、停留时间、搜索��键词等行为数据，动态调整引导内容和产品推荐。为新用户提供专属新人礼包（���首单优惠券、会员积分奖励）、个性化商品推荐清单、平台核心功能介绍等，帮助用户快速了解平台价值并找到感兴趣的内容。同时收集用户偏好标签，为后续个性化营销奠定基础。'
    },
    {
