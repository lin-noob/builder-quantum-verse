...
import { Plus, Edit, Trash2, GripVertical, HelpCircle, Check } from "lucide-react";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";
import { EventRule, EventType, NamedEvent, RawEventType } from "@shared/eventRuleTypes";
...
const RulesPage = () => {
  ...
  const saveRule = async () => {
    ...
    const hasTextAliases = editingRule.conditions.text?.aliases?.some((alias) => alias.trim() !== "");
    const hasTitleIncludes = editingRule.conditions.pageTitleIncludes?.some((title) => title.trim() !== "");
  ...
  const [formSuperposition, setFormSuperposition] = useState(false); // 叠加标识开关
  // 事件标识选项与细化标识管理
  ...
  const [detailMap, setDetailMap] = useState<
    Record<string, Array<{ key: string; value: string; superPosition?: boolean }>>
  >({});
  ...
  const [newDetailSuperposition, setNewDetailSuperposition] = useState(false);
  ...
  const openCreateOutcomeDrawer = () => {
    ...
    setDetailEditKey("");
    setDetailEditValue("");
    setNewDetailSuperposition(false);
  ...
  const openEditOutcomeDrawer = (eventId: string) => {
    ...
    setFormSuperposition(event.superPosition || false);

    try {
      const attributesData = event.attributes ? JSON.parse(event.attributes) : null;
  ...
  const saveOutcomeEvent = async () => {
    ...
    if (!editingOutcomeId) {
      ...
      try {
        ...
        toast({
          title: "保存失败",
          description: error instanceof Error ? error.message : "创建结果事件失败，请稍后重试",
          variant: "destructive",
        });
      }
    } else {
      ...
      try {
        ...
        toast({
          title: "更新失败",
          description: error instanceof Error ? error.message : "更新结果事件失败，请稍后重试",
          variant: "destructive",
        });
      }
    }
  ...
  const deleteResultEvent = async (eventId: string) => {
    try {
      ...
      toast({
        title: "删除失败",
        description: error instanceof Error ? error.message : "删除结果事件失败，请稍后重试",
        variant: "destructive",
      });
    }
  };
  ...
  // 结果事件列表拖拽（调用后端排序）
  const handleOutcomeDragStart = (e: React.DragEvent<HTMLDivElement>, eventId: string) => {
  ...
  const handleOutcomeDrop = async (e: React.DragEvent<HTMLDivElement>, targetIndex: number) => {
    e.preventDefault();
    const draggedEventId = e.dataTransfer.getData("text/plain");

    if (draggedEventId && draggedOutcome) {
      const draggedIndex = resultEvents.findIndex((event) => event.id === draggedEventId);
      if (draggedIndex !== -1 && draggedIndex !== targetIndex) {
        ...
        if (sourceEvent.id && targetEvent.id) {
          try {
            ...
            toast({
              title: "错误",
              description: error instanceof Error ? error.message : "移动结果事件失败",
              variant: "destructive",
            });
            setDraggedOutcome(null);
            return;
                    ...
                    <CardContent className="py-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge variant={rule.enabled ? "default" : "secondary"}>{rule.targetEvent}</Badge>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex items-center h-10 !mt-0">
                                <Switch
                                  ...
                                  onCheckedChange={(v) => updateEditing({ enabled: v })}
                                />
                              </div>
                            </div>
                          </div>
                        ...
                        <CardContent className="space-y-4">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <Tooltip delayDuration={300}>
                                  ...
                                  <TooltipContent
                                    ...
                                    className="max-w-xs bg-gray-900 text-white border-gray-700"
                                  >
                                    与范围类型配合使用的规则体匹配值：如 /auth、/order（前缀），或
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center gap-2">
                              ...
                              <Tooltip delayDuration={300}>
                                ...
                                <TooltipContent side="top" className="max-w-xs bg-gray-900 text-white border-gray-700">
                                  定义多个文本关键词，用于匹配页面中的文本内容,
                                  识别页面中包含特定文本的元素（如按钮文字、链接文字等）。例如：设置["提交", "Submit",
                                  "Send"]，匹配包含这些文字的元素。
                                </TooltipContent>
                              </Tooltip>
                            </div>
                            {(editingRule.conditions.text?.aliases || []).map((v, idx) => (
                              <div key={idx} className="flex gap-2 mb-2">
                                ...
                                <Button variant="ghost" size="sm" onClick={() => removeAlias(idx)}>
                                  删除
                                </Button>
                              </div>
                            ))}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                              <div className="space-y-2">
                                ...
                                <Select
                                  value={editingRule.conditions.text?.matchMode || "contains"}
                                  onValueChange={(v) =>
                                    updateEditing({
                                      conditions: {
                                        ...editingRule.conditions,
                                        text: {
                                          aliases: editingRule.conditions.text?.aliases || [],
                                          matchMode: v as any,
                                        },
                                      },
                                    })
                          ...
                          <div className="space-y-4">
                            <div className="space-y-2">
                              ...
                              <Input
                                value={editingRule.conditions.selector?.selector || ""}
                                onChange={(e) =>
                                  updateEditing({
                                    conditions: {
                                      ...editingRule.conditions,
                              ...
                              <Input
                                ...
                                value={
                                  editingRule.conditions.selector?.attributesRaw !== undefined
                                    ...
                                    : Object.entries(editingRule.conditions.selector?.attributes || {})
                                        .map(([k, v]) => `${k}=${v}`)
                                        .join(",")
                                }
                                onChange={(e) => {
                    ...
                    >
                      <CardContent className="py-3 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div
                            draggable
                            onDragStart={(e) => handleOutcomeDragStart(e, event.id)}
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">{event.remark}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Badge variant={event.enableFlag ? "default" : "secondary"}>
                            {baseCodeMap[event.eventType]}
                          </Badge>
                          <Badge variant="outline">{event.enableFlag ? "启用" : "禁用"}</Badge>
                          <Button variant="ghost" size="sm" onClick={() => openEditOutcomeDrawer(event.id)}>
                            <Edit className="h-4 w-4 mr-1" /> 编辑
                          </Button>
                          <Button variant="ghost" size="icon" onClick={() => deleteResultEvent(event.id)}>
                            <Trash2 className="h-4 w-4" />
                          </Button>
                        </div>
                      </CardContent>
                      ...
                      <div className="grid grid-cols-4 gap-3 items-center">
                        ...
                        <div className="col-span-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-muted-foreground">{formEnabled ? "启用" : "禁用"}</span>
                          </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-12 gap-2">
                          ...
                          <div className="col-span-2 flex items-center justify-center gap-2">
                            ...
                            <Switch
                              checked={newDetailSuperposition}
                              disabled={!formSuperposition}
                              onCheckedChange={setNewDetailSuperposition}
                              className="scale-75"
                            />
                          </div>
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setDetailMap((prev) => {
                                ...
                                const exists = list.some((item) => item.key === key && item.value === value);
                                if (!exists) {
                                  // Mutex logic: if new one is superPosition, disable others
                                  if (newDetailSuperposition) {
                                    list.forEach((item) => (item.superPosition = false));
                                  }
                                  list.push({ key, value, superPosition: newDetailSuperposition });
                                }
                                ...
                                {editingDetailIndex === idx ? (
                                  <div className="flex-1 flex gap-2">
                                    <Input
                                      placeholder="键"
                                      value={detailEditKey}
                                      onChange={(e) => setDetailEditKey(e.target.value)}
                                    />
                                    <Input
                                      placeholder="值"
                                  ...
                                  >
                                    <div
                                      className={`font-mono text-sm ${
                                        selectedDetail?.key === d.key && selectedDetail?.value === d.value
                                          ? "text-primary font-semibold"
                                          : ""
                                      }`}
                                    >
                                      <span className="text-blue-600">{d.key}</span> ={" "}
                                      <span className="text-green-600">{d.value || '""'}</span>
                                    </div>
                                    <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                      <span className="text-xs text-muted-foreground">叠加</span>
                                      <Switch
                                        checked={!!d.superPosition}
                                        disabled={!formSuperposition}
                                        onCheckedChange={(checked) => {
                                          setDetailMap((prev) => {
                                            const list = [...(prev[formCode || ""] || [])];
                                            ...
                                            if (checked) {
                                              list.forEach((item, i) => {
                                                if (i === idx) {
                                                  item.superPosition = true;
                                                } else {
                                                  item.superPosition = false;
                                                }
                                              });
                                            } else {
                                              list[idx] = {
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <Button
                                      ...
                                      onClick={() => {
                                        ...
                                        if (selectedDetail?.key === d.key && selectedDetail?.value === d.value)
                                          setSelectedDetail(null);
                                      }}
                                    >
                                      <Trash2 className="h-4 w-4" />
