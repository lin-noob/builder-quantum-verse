...
import { Link as LinkIcon, Copy, Check, RotateCcw, Trash2, Search, ExternalLink, Plus } from "lucide-react";
...
export default function UTMGenerator() {
  ...
  useEffect(() => {
    try {
      ...
      const now = Date.now();
      const mock: UTMRecord[] = [
        {
          id: `${now - 1}`,
          title: "春季促销",
          finalUrl:
            "https://www.acme.com/landing?utm_source=google&utm_medium=cpc&utm_campaign=spring_sale&utm_term=sale&utm_content=banner_A",
          targetUrl: "https://www.acme.com/landing",
          utm_source: "google",
          utm_medium: "cpc",
          utm_campaign: "spring_sale",
          utm_term: "sale",
          utm_content: "banner_A",
          extraParams: [{ key: "adgroup", value: "brand" }],
        ...
        },
        {
          ...
          title: "公众号投放",
          finalUrl:
            "https://www.acme.com/promo?utm_source=wechat&utm_medium=social&utm_campaign=brand_awareness_2024&utm_content=article_1",
          targetUrl: "https://www.acme.com/promo",
          utm_source: "wechat",
          utm_medium: "social",
          utm_campaign: "brand_awareness_2024",
          utm_content: "article_1",
        ...
        },
        {
          ...
          title: "双十一预热",
          finalUrl:
            "https://shop.acme.com/?utm_source=taobao&utm_medium=affiliate&utm_campaign=double11_preheat",
          targetUrl: "https://shop.acme.com/",
          utm_source: "taobao",
          utm_medium: "affiliate",
          utm_campaign: "double11_preheat",
          createdAt: new Date(now - 15 * 24 * 60 * 60 * 1000).toISOString(),
          archived: true,
        },
      ];
      setHistory(mock);
      localStorage.setItem(HISTORY_STORAGE_KEY, JSON.stringify(mock));
    } catch {}
  }, []);
  const persist = (items: UTMRecord[]) => { setHistory(items); try { localStorage.setItem(HISTORY_STORAGE_KEY, JSON.stringify(items)); } catch {} };
  ...
  const saveHistory = () => {
    if (!finalUrl || urlError || requiredError) { toast({ title: "无法保存", description: "请修复必填项与 URL" , variant: "destructive"}); return; }
    const rec: UTMRecord = {
      id: `${Date.now()}`,
      ...
      extraParams: extraParams.filter(p => p.key && p.value),
  ...
  const fillBack = (r: UTMRecord) => {
    setTargetUrl(r.targetUrl); setSource(r.utm_source); setMedium(r.utm_medium); setCampaign(r.utm_campaign); setTerm(r.utm_term || ""); setContent(r.utm_content || ""); setExtraParams(r.extraParams?.length ? r.extraParams : [{ key: "", value: "" }]); toast({ title: "已填回表单" }); window.scrollTo({ top: 0, behavior: "smooth" });
  };
  const del = (id: string) => { persist(history.filter(h => h.id !== id)); toast({ title: "已删除" }); };
  const toggleArchive = (id: string) => { persist(history.map(h => h.id === id ? { ...h, archived: !h.archived } : h)); };

  const filtered = useMemo(() => {
    const t = searchText.trim().toLowerCase();
    return history.filter(h => {
      const matchTerm = !t || h.title.toLowerCase().includes(t) || h.finalUrl.toLowerCase().includes(t) || h.utm_source.toLowerCase().includes(t) || h.utm_medium.toLowerCase().includes(t) || h.utm_campaign.toLowerCase().includes(t);
      const matchArchived = showArchived === null || (!!h.archived) === showArchived;
      return matchTerm && matchArchived;
    });
  }, [history, searchText, showArchived]);
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="px-6 py-4">标题</TableHead>
              ...
              <TableBody>
                {filtered.length === 0 ? (
                  <TableRow><TableCell colSpan={8} className="px-6 py-8 text-center text-gray-500">暂无历史记录</TableCell></TableRow>
                ) : (
                  filtered.map(h => (
                    <TableRow key={h.id}>
                      <TableCell className="px-6 py-4 font-medium">{h.title}</TableCell>
                      <TableCell className="px-6 py-4 text-xs break-all">{h.targetUrl}</TableCell>
                      <TableCell className="px-6 py-4">{h.utm_source}</TableCell>
                      <TableCell className="px-6 py-4">{h.utm_medium}</TableCell>
                      <TableCell className="px-6 py-4">{h.utm_campaign}</TableCell>
                      <TableCell className="px-6 py-4 text-xs">{new Date(h.createdAt).toLocaleString()}</TableCell>
                      <TableCell className="px-6 py-4">{h.archived ? <Badge variant="outline">已归档</Badge> : <Badge>活跃</Badge>}</TableCell>
                      <TableCell className="px-6 py-4">
                        <div className="flex flex-wrap gap-2">
                          <Button variant="outline" size="sm" onClick={() => copyText(h.finalUrl)}><Copy className="h-4 w-4 mr-2" />复制</Button>
                          <Button variant="outline" size="sm" onClick={() => fillBack(h)}>填回</Button>
                          <Button variant="outline" size="sm" onClick={() => toggleArchive(h.id)}>{h.archived ? "取消归档" : "归档"}</Button>
