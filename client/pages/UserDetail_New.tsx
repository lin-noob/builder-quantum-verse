...
import { getProfileView, type ApiUser, addProfileLabel, deleteProfileLabel, ApiLabel } from "@/lib/profile";
...
export default function UserDetail() {
  ...
  const getChannelType = useMemo(() => {
    ...
    const isPaidMedium = paidMediums.some((paidType) => medium.toLowerCase().includes(paidType.toLowerCase()));
    if (isPaidMedium) {
      return { type: "付费广告", color: "bg-red-100 text-red-800" };
    }
    ...
    const isFromSearchEngine = searchEngines.some((engine) => referrer.toLowerCase().includes(engine));
    // 确保不是付费媒介且来自搜索引擎
    ...
    return [
      {
        key: "referrer",
        label: "Referrer",
        hint: "HTTP 引用来源，标识用户是从哪个网页链接点击进入当前页面的完整 URL",
        value: apiUser?.firstReferrer || "",
      },
      {
        key: "utm_source",
        label: "utm_source",
        hint: "流量来源，标识用户是从哪个网站或平台来的（如 google、facebook、newsletter）",
        value: apiUser?.utm_source || "",
      },
      {
        key: "utm_medium",
        label: "utm_medium",
        hint: "流量媒介，标识用户是通过什么方式来的（如 cpc、email、social）",
        value: apiUser?.utm_medium || "",
      },
      {
        key: "utm_campaign",
        label: "utm_campaign",
        hint: "营销活动名称，标识具体的推广活动（如 summer_sale、product_launch）",
        value: apiUser?.utm_campaign || "",
      },
      {
        key: "utm_content",
        label: "utm_content",
        hint: "广告内容标识，用于区分同一活动中不同版本或位置的广告（如 banner_a、link_b）",
        value: apiUser?.utm_content || "",
      },
      {
        key: "utm_term",
        label: "utm_term",
        hint: "关键词，主要用于付费搜索广告，标识用户搜索的关键词（如 running+shoes）",
        value: apiUser?.utm_term || "",
      },
      {
        key: "gclid",
        label: "gclid",
        hint: "Google Ads 自动标记参数，用于追踪 Google 广告点击",
        value: apiUser?.gclid || "",
      },
      ...
      { key: "gclsrc", label: "gclsrc", hint: "Google 广告来源标识符，用于标识广告来源", value: apiUser?.gclsrc || "" },
      {
        key: "gbraid/wbraid",
        label: "gbraid/wbraid",
        hint: "Google 隐私保护型广告点击标识符，用于替代 gclid（iOS 14 之后）",
        value: gbraidWbraid || "",
      },
      ...
      // { key: "title", label: "title", hint: "页面或活动的标题，用于标识当前页面或活动名称", value: apiUser?.title || "" },
      {
        key: "fbclid",
        label: "fbclid",
        hint: "Facebook 广告点击标识符，用于追踪 Facebook 广告点击",
        value: apiUser?.fbclid || "",
      },
      {
        key: "msclkid",
        label: "msclkid",
        hint: "Microsoft Advertising（Bing）广告点击标识符",
        value: apiUser?.msclkid || "",
      },
      {
        key: "ttclid",
        label: "ttclid",
        hint: "TikTok 广告点击标识符，用于追踪 TikTok 广告点击",
        value: apiUser?.ttclid || "",
      },
      {
        key: "twclid",
        label: "twclid",
        hint: "Twitter 广告点击标识符，用于追踪 Twitter 广告点击",
        value: apiUser?.twclid || "",
      },
      { key: "qclid", label: "qclid", hint: "可能是某个广告平台的点击标识符，非标准字段", value: apiUser?.qclid || "" },
      {
        key: "dclid",
        label: "dclid",
        hint: "DoubleClick 广告点击标识符，用于追踪 DoubleClick 广告点击",
        value: apiUser?.dclid || "",
      },
      {
        key: "rdt_cid",
        label: "rdt_cid",
        hint: "Reddit 广告点击标识符，用于追踪 Reddit 广告点击",
        value: apiUser?.rdt_cid || "",
      },
      {
        key: "irclid",
        label: "irclid",
        hint: "Impact Radius 广告点击标识符，用于追踪 Impact Radius 广告点击",
        value: apiUser?.irclid || "",
      },
      {
        key: "li_fat_id",
        label: "li_fat_id",
        hint: "LinkedIn 广告点击标识符，用于追踪 LinkedIn 广告点击",
        value: apiUser?.li_fat_id || "",
      },
      {
        key: "mc_cid",
        label: "mc_cid",
        hint: "Mailchimp 活动 ID，用于追踪 Mailchimp 邮件活动",
        value: apiUser?.mc_cid || "",
      },
      {
        key: "sccid",
        label: "sccid",
        hint: "Snapchat 广告点击标识符，用于追踪 Snapchat 广告点击",
        value: apiUser?.sccid || "",
      },
      {
        key: "igshid",
        label: "igshid",
        hint: "Instagram 分享或广告点击标识符，用于追踪 Instagram 相关行为",
        value: apiUser?.igshid || "",
      },
      {
        key: "_kx",
        label: "_kx",
        hint: "可能是某个内部系统或第三方工具的标识符，非标准字段",
        value: apiUser?._kx || "",
      },
      {
        key: "epik",
        label: "epik",
        hint: "可能是某个广告平台或内部系统的标识符，非标准字段",
        value: apiUser?.epik || "",
      },
    ];
  }, [apiUser, user]);
                  ...
                  <Card id="overview">
                    <CardContent className="p-6">
                      <div className="space-y-4">
                        <h2 className="text-xl font-semibold">{user.name || String(user.cdpId).substring(0, 8)}</h2>

                        <div className="flex items-center gap-3">
                          <User className="h-4 w-4 text-gray-500" />
                          <span className="text-sm text-gray-600">CDP ID:</span>
                          <code className="text-sm font-mono bg-gray-100 px-2 py-1 rounded">{user.userId}</code>
                        ...
                        <div className="bg-white border border-gray-200 rounded-lg overflow-visible">
                          <div className="overflow-x-auto">
                            <table className="min-w-full table-fixed">
                              ...
                              <tbody className="divide-y divide-gray-200">
                                {(() => {
                                  const fields = showAllFirstVisitFields
                                    ? firstVisitFieldDefs
                                    : firstVisitFieldDefs.slice(0, 8);
                                  const rows = [];
                            ...
                            <table className="min-w-full table-fixed">
                              ...
                              <tbody className="divide-y divide-gray-200">
                                ...
                                <tr>
                                  <td className="px-4 py-3 text-sm text-gray-900 bg-gray-50">
                                    近7天事件数
                                    <TooltipIcon text="最近7天内的总事件数" />
                                  </td>
                                  <td className="px-4 py-3 text-sm text-gray-900">
                                    {user?.userEngagement?.eventCount7d ?? 0}
                                  </td>
                                  <td className="px-4 py-3 text-sm text-gray-900 bg-gray-50">
                                    页面浏览量
                                    <TooltipIcon text="累计浏览的页面总数" />
                                  </td>
                                  <td className="px-4 py-3 text-sm text-gray-900">
                                    {user?.userEngagement?.pageViewTotal ?? 0}
                                  </td>
                                </tr>
                                <tr>
                                  <td className="px-4 py-3 text-sm text-gray-900 bg-gray-50">
                                    近30天事件数
                                    <TooltipIcon text="最近30天内的总事件数" />
                                  </td>
                                  <td className="px-4 py-3 text-sm text-gray-900">
                                    {user?.userEngagement?.eventCount30d ?? 0}
                                  </td>
                                  <td className="px-4 py-3 text-sm text-gray-900 bg-gray-50">
                                    近30天页面浏览
                                    <TooltipIcon text="最近30天内浏览的页面总数" />
                                  </td>
                                  <td className="px-4 py-3 text-sm text-gray-900">
                                    {user?.userEngagement?.pageView30d ?? 0}
                                  </td>
                                </tr>
                              </tbody>
                            </table>
