...
const EnterpriseModelOverview = () => {
  ...
  const [counts, setCounts] = useState({
    entities: { objectCount: 0, relationCount: 0, mainId: "" },
    capabilities: { objectCount: 0, relationCount: 0, mainId: "" },
    rules: { objectCount: 0, relationCount: 0, mainId: "" },
  });
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const [entityRes, capabilityRes, ruleRes] = await Promise.all([
          request.get("/quote/api/v1/digital-model/count", { modelId: 1 }),
          request.get("/quote/api/v1/digital-model/count", { modelId: 2 }),
          request.get("/quote/api/v1/digital-model/count", { modelId: 3 }),
        ]);

        setCounts({
          entities: entityRes.data?.data || { objectCount: 0, relationCount: 0, mainId: "" },
          capabilities: capabilityRes.data?.data || { objectCount: 0, relationCount: 0, mainId: "" },
          rules: ruleRes.data?.data || { objectCount: 0, relationCount: 0, mainId: "" },
        });
      } catch (error) {
    ...
    () => [
      {
        ...
        icon: Database,
        count: counts.entities.objectCount,
        status: counts.entities.objectCount > 10 ? "完成" : counts.entities.objectCount > 0 ? "部分完成" : "未配置",
        mainText: "定义企业核心数据结构及其内在生命周期。",
        hoverText: "包含客户、订单、商品等核心业务对象的元数据定义、数据库映射及生命周期状态机。",
        category: "digital",
        metrics: [
          { label: "已定义对象", value: String(counts.entities.objectCount) },
          { label: "关联表总数", value: String(counts.entities.relationCount) },
        ],
      },
      {
        ...
        icon: Plug,
        count: counts.capabilities.objectCount,
        status:
          counts.capabilities.objectCount > 10 ? "完成" : counts.capabilities.objectCount > 0 ? "部分完成" : "未配置",
        mainText: "可被流程编排调用的标准化业务执行单元（原子算子）。",
        hoverText: "将业务动作封装为标准化的输入输出接口，如“创建订单”、“冻结账户”，供流程引擎调用。",
        category: "digital",
        metrics: [
          { label: "API 覆盖率", value: String(counts.capabilities.objectCount) },
          { label: "可用插件", value: String(counts.capabilities.objectCount) },
        ],
      },
      {
        ...
        icon: ShieldCheck,
        count: counts.rules.objectCount,
        status: counts.rules.objectCount > 10 ? "完成" : counts.rules.objectCount > 0 ? "部分完成" : "未配置",
        mainText: "跨流程的全局约束、风险拦截与合规准则。",
        hoverText: "定义不可被违反的底线规则，如“单笔交易限额”、“GDPR 隐私合规”，自动拦截违规操作。",
        category: "digital",
        metrics: [
          { label: "生效规则", value: String(counts.rules.objectCount) },
          { label: "拦截等级", value: counts.rules.objectCount > 0 ? "High" : "None" },
        ],
      },
  ...
  const handleNavigate = (moduleId: string) => {
    ...
    if (path) {
      navigate(mid ? `${path}?mainId=${mid}` : path);
    } else {
              ...
              <Dialog open={isVersionDialogOpen} onOpenChange={setIsVersionDialogOpen}>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>创建一个新的模型版本快照。当前所有配置将被保存到新版本中。</DialogDescription>
                  </DialogHeader>
                ...
                <ScrollArea className="flex-1 -mx-2 px-2">
                  <div className="grid grid-cols-3 gap-6 pb-4">
                    {currentModules.map((module) => (
                      <Card
                        key={module.id}
                        className="hover:shadow-lg transition-all duration-300 border-gray-100 bg-white/80 backdrop-blur-sm hover:-translate-y-1 flex flex-col h-full text-left group cursor-pointer"
                      ...
                      >
                        ...
                        <CardContent className="flex-1 flex flex-col justify-between">
                          <div className="space-y-4">
                            <p className="text-sm text-slate-500 leading-relaxed min-h-[40px]">{module.mainText}</p>
                            ...
                            {module.metrics && (
                              <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-100">
                                {module.metrics.map((metric, idx) => (
                                  <div key={idx}>
                                    <div className="text-xs text-slate-400 uppercase tracking-wide scale-90 origin-left">
                                      {metric.label}
                                    </div>
