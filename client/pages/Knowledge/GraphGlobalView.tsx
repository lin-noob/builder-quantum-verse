...
const TYPE_COLORS = {
  ...
  Transaction: { bg: 'bg-purple-50', border: 'border-purple-500', text: 'text-purple-900', badge: 'bg-purple-100 text-purple-700' },
  Result: { bg: 'bg-emerald-50', border: 'border-emerald-500', text: 'text-emerald-900', badge: 'bg-emerald-100 text-emerald-700' }
};

// --- Custom Node Component ---
const CustomNode = ({ data, selected }: { data: { label: string; type: KnowledgeNodeType; risk: boolean; stats: any }; selected: boolean }) => {
...
const GraphGlobalView: React.FC = () => {
  ...
  if (loading) return <div className="h-full w-full flex items-center justify-center bg-slate-50 text-slate-400">正在构建企业知识图谱...</div>;
      ...
      <div className="flex-1 relative h-full">
        ...
        <ReactFlowProvider>
          ...
          >
            ...
            <Controls className="bg-white border-slate-200 shadow-sm rounded-lg overflow-hidden m-4" showInteractive={false} />
            <MiniMap 
            ...
            <div className="flex-1 overflow-y-auto p-5">
               ...
               <div className="space-y-3">
                 <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-100">
                   ...
                   <span className="font-bold text-slate-900">{selectedNodeData.attributeCount || selectedNodeData.properties.length}</span>
                 </div>
                 ...
                 <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-100">
                   ...
                   <span className="font-bold text-slate-900">{selectedNodeData.relationCount || selectedNodeData.relations.length}</span>
                 </div>
                 ...
                 <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-100">
                   ...
                   <span className="font-bold text-slate-900">{selectedNodeData.actionCount || selectedNodeData.actions.length}</span>
                 </div>
               </div>
