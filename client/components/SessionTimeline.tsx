...
const sessionsData = [
  {
    ...
    event_count: 3,
    events: [
      { event_time: '14:30:15', event_type: 'page_view', page_title: '首页 - 我们的网站', page_url: '/', page_dwell_time: '1分20秒', page_scroll_depth: '85%' },
      { event_time: '14:31:35', event_type: 'page_view', page_title: '产品列表 - 电子产品', page_url: '/products', page_dwell_time: '5分10秒', page_scroll_depth: '100%' },
      { event_time: '14:36:45', event_type: 'add_to_cart', page_title: '将"ProBook X1"加入购物车', page_url: '/products/probook-x1', page_dwell_time: 'N/A', page_scroll_depth: 'N/A' },
    ]
  },
  {
    sessionId: 'session-2',
    ...
    event_count: 4,
    events: [
      { event_time: '10:05:30', event_type: 'page_view', page_title: '购物车', page_url: '/cart', page_dwell_time: '3分', page_scroll_depth: '60%' },
      { event_time: '10:08:30', event_type: 'start_checkout', page_title: '开始结账流程', page_url: '/checkout', page_dwell_time: 'N/A', page_scroll_depth: 'N/A' },
      { event_time: '10:12:00', event_type: 'page_view', page_title: '支付页面', page_url: '/checkout/pay', page_dwell_time: '3分', page_scroll_depth: '100%' },
      { event_time: '10:15:00', event_type: 'purchase', page_title: '完成订单 ORD-20250801-001', page_url: '/thank-you', page_dwell_time: 'N/A', page_scroll_depth: 'N/A' },
    ]
  },
  {
    sessionId: 'session-3',
    ...
    event_count: 1,
    events: [
      { event_time: '09:15:00', event_type: 'page_view', page_title: '夏季促销活动页', page_url: '/promo/summer-sale', page_dwell_time: '5分钟', page_scroll_depth: '95%' },
    ]
  }
];
...
export default function SessionTimeline() {
  ...
  const getEventTypeBadge = (eventType: string) => {
    const badgeClass = "px-2 py-0.5 text-xs font-medium bg-slate-200 text-slate-700 rounded-full";
  ...
  const handleRowClick = (e: Event) => {
    ...
    const sessionData = sessionsData.find(session => session.sessionId === sessionId);
  ...
  const populateModal = (session: any) => {
    // 填充访问概览
    const modalSessionSummary = document.getElementById('modal-session-summary');
    ...
    if (modalEventTimeline) {
      modalEventTimeline.innerHTML = session.events.map((event: any) => `
        <tr>
      ...
      >
        <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
          ...
          <div className="p-6 overflow-y-auto">
            {/* 访问概览区 */}
            <div className="mb-6">
              <h4 className="text-sm font-medium text-slate-900 mb-3">访问概览</h4>
              <div 
            ...
            {/* 事件时间线区 */}
            <div>
              <h4 className="text-sm font-medium text-slate-900 mb-3">事件时间线</h4>
              <div className="border border-slate-200 rounded-lg overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="p-3 text-left font-medium text-slate-500">事件时间</th>
                      <th className="p-3 text-left font-medium text-slate-500">事件类型</th>
                      <th className="p-3 text-left font-medium text-slate-500">页面/事件描述</th>
                      <th className="p-3 text-left font-medium text-slate-500">停留时长</th>
                      <th className="p-3 text-left font-medium text-slate-500">滚动深度</th>
                    </tr>
                  </thead>
                  <tbody id="modal-event-timeline" className="divide-y divide-slate-200">
                    {/* 事件行将由 JavaScript 填充 */}
                  </tbody>
                </table>
              </div>
