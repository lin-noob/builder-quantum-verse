...
export const DecisionTraceLayout: React.FC<DecisionTraceLayoutProps> = ({ eventId }) => {
  const [state, setState] = useState<DecisionTraceState>(() => getMockState(eventId));
  ...
  const goToNextStep = () => {
    ...
    if (next <= 5) {
      ...
      setState(prev => ({ ...prev, currentStep: stepMap[next] }));
    }
  };
  ...
  const headerEvent: DecisionEvent = useMemo(() => {
    return {
      ...
      ai_analyzed_at: new Date(new Date(state.triggerEvent.timestamp).getTime() + 60000).toISOString().replace('T', ' ').substring(0, 16),
              ...
              return (
                <React.Fragment key={step.id}>
                  ...
                  {index < steps.length - 1 && (
                    <div className={cn(
                      "w-8 h-[2px] mx-1",
                      step.id < maxReachedStep ? "bg-blue-200" : "bg-slate-200"
                    )} />
                  )}
                </React.Fragment>
              );
