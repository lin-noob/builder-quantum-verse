...
import { useRoles, useCreateRole, useUpdateRole, useDeleteRole } from "@/admin/hooks/useRoleManagement";
...
export default function GranularPermissionManagement({ 
  ...
  const total = rolesData?.total || mockRoles.length;
  ...
  const toggleModule = (moduleId: string) => {
    ...
    // 检查是否所有功能都已选中
    const allPermissionsSelected = modulePermissions.every(id => currentPermissions.includes(id));
    
    let updatedPermissions;
    if (allPermissionsSelected) {
      // 如果所有功能都已选中，则取消选中所有功能
      updatedPermissions = currentPermissions.filter(id => !modulePermissions.includes(id));
    } else {
  ...
  const toggleSubModule = (resource: string) => {
    ...
    // 检查是否所有功能都已选中
    const allPermissionsSelected = resourcePermissions.every(id => currentPermissions.includes(id));
    
    let updatedPermissions;
    if (allPermissionsSelected) {
      // 如果所有功能都已选中，则取消选中所有功能
      updatedPermissions = currentPermissions.filter(id => !resourcePermissions.includes(id));
    } else {
      // 如果不是所有功能都已选中，则选中所有功能
      updatedPermissions = [...currentPermissions];
  ...
  const isModulePartiallySelected = (moduleId: string) => {
    ...
    // 确保 permissionIds 存在
    const currentPermissions = selectedRole.permissionIds || [];
    const selectedCount = modulePermissions.filter(id => currentPermissions.includes(id)).length;
  ...
  const isSubModuleFullySelected = (resource: string) => {
    ...
    // 确保 permissionIds 存在
    const currentPermissions = selectedRole.permissionIds || [];
    return resourcePermissions.every(id => currentPermissions.includes(id));
  };
                                    ...
                                    {mockPermissions
                                      ...
                                      .map((permission) => (
                                        <div key={permission.id} className="flex items-center space-x-2 p-3 border rounded-lg">
                                          <Checkbox
                                            id={permission.id}
                                            checked={selectedRole?.permissionIds?.includes(permission.id) || false}
