...
const EVENT_FIELDS: FieldConfig[] = [
  {
    ...
    name: "页面URL",
    dataType: "text",
    description: "事件发生时所在页面的网址",
    applicableEvents: ["page_view"],
  },
  {
    id: "page_title",
    name: "页面标题",
    dataType: "text",
    description: "事件发生时所在页面的标题",
    applicableEvents: ["page_view"],
  },
  {
    id: "page_dwell_time_seconds",
    name: "页面停留时长(秒)",
    dataType: "number",
    description: "用户在该页面已停留的总秒数",
    applicableEvents: ["page_view"],
  },
  {
    id: "page_scroll_depth",
    name: "页面滚动深度(%)",
    dataType: "number",
    description: "用户在该页面向下的最大滚动位置",
    applicableEvents: ["page_view"],
  },
  {
    id: "product_name",
    ...
    dataType: "text",
    description: "相关的商品名称",
    applicableEvents: ["view_product", "add_to_cart", "remove_from_cart"],
  },
  {
    id: "category",
    name: "商品品类",
  ...
  },
  {
    ...
    dataType: "number",
    description: "购物车商品总价",
    applicableEvents: ["add_to_cart", "remove_from_cart"],
  },
  {
    id: "form_name",
    name: "表单名称/ID",
    dataType: "text",
    description: "提交的表单名称或ID",
    applicableEvents: ["submit_form"],
  },
  {
    id: "search_term",
    name: "搜索关键词",
...
const USER_FIELDS: FieldConfig[] = [
  {
    ...
    name: "用户标签",
    dataType: "text",
    description: "用户所拥有的标签",
  },
  {
    id: "total_spend",
    name: "累计消费金额",
    dataType: "number",
    description: "用户历史累计的总消费",
  },
  {
    id: "total_orders",
    name: "总订单数",
    dataType: "number",
    description: "用户历史累计的总订单数",
  },
  {
    id: "last_purchase_days",
    name: "距上次购买天数",
    dataType: "number",
    description: "距离该用户上一次购买过去了多少天",
  },
];
...
export default function TriggerRuleConfigNew({
  value,
  onChange,
  className = "",
}: TriggerRuleConfigProps) {
  const [selectedEvent, setSelectedEvent] = useState<TriggerEventName>(
    value.config.eventName,
  );
  const [conditions, setConditions] = useState<ConditionItem[]>(
    value.config.conditions || [],
  );
  const [userConditions, setUserConditions] = useState<ConditionItem[]>(
    value.config.userConditions || [],
  );
  ...
  const getAvailableEventFields = (): FieldConfig[] => {
    return EVENT_FIELDS.filter(
      (field) =>
        !field.applicableEvents ||
        field.applicableEvents.includes(selectedEvent),
    );
  };
            ...
            >
              ...
              <div className="flex-1">
                <Select
                  ...
                  onValueChange={(value) =>
                    updateCondition(type, index, { field: value })
                  }
                >
            ...
            <TabsContent value="event" className="space-y-4 mt-4">
              ...
              {getAvailableEventFields().length > 0 ? (
                ...
                <div className="text-sm text-gray-500 italic bg-yellow-50 border border-yellow-200 p-3 rounded-lg">
                  ⚠️ 当前触发事件"{EVENT_DISPLAY_NAMES[selectedEvent]}
                  "无需设置事件属性条件
                </div>
              )}
            </TabsContent>
        ...
        <CardContent>
          <div className="space-y-3">
            ...
            {conditions.length > 0 && (
              <div>
                <Badge variant="outline" className="mb-2">
                  事件属性条件 ({conditions.length})
                </Badge>
                <div className="text-sm text-gray-700 space-y-1">
                  {conditions.map((condition, index) => {
                    const field = getAvailableEventFields().find(
            ...
            {userConditions.length > 0 && (
              <div>
                <Badge variant="outline" className="mb-2">
                  用户画像条件 ({userConditions.length})
                </Badge>
                <div className="text-sm text-gray-700 space-y-1">
                  {userConditions.map((condition, index) => {
                    const field = USER_FIELDS.find(
                      (f) => f.id === condition.field,
                    );
                    return (
                      <div key={index} className="pl-2">
