...
export function OperationLogsTab({ organizationId, organizationName }: OperationLogsTabProps) {
  ...
  useEffect(() => {
    // 模拟数据加载 - 实际应用中应该调用API获取特定组织的操作日志
    setTimeout(() => {
      // 过滤出当前组织的操作记录
      const orgRecords = mockOperationRecords.filter(
        record => record.organization === organizationName
      );
      setOperationRecords(orgRecords);
      setFilteredRecords(orgRecords);
      setLoading(false);
    }, 500);
  ...
  useEffect(() => {
    ...
    // 重置到第一页
    setCurrentPage(1);
  }, [searchTerm, operationRecords, sortConfig, statusFilter, moduleFilter]);
