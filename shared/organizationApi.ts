...
export const organizationApi = {
  ...
  async getOrganizationById(
    ...
    const organization = organizationsStore.find(
  ...
  ): Promise<ApiResponse<{ organization: Organization; admin: Member }>> {
    await delay(500);
  ...
  async updateOrganization(
    ...
    const index = organizationsStore.findIndex(
...
export const memberApi = {
  ...
  async getMembers(
    ...
    let filteredMembers = membersStore.filter(
    ...
    if (status) {
      filteredMembers = filteredMembers.filter(
        (member) => member.accountStatus === status,
      );
    }
  ...
  async inviteMember(
  ...
  ): Promise<ApiResponse<{ member: Member; initialPassword: string }>> {
    await delay(500);
    ...
    // 更新组织成员数量
    const orgIndex = organizationsStore.findIndex(
      (org) => org.organizationId === organizationId,
    );
    if (orgIndex !== -1) {
      organizationsStore[orgIndex].memberCount =
        (organizationsStore[orgIndex].memberCount || 0) + 1;
      organizationsStore[orgIndex].activeMemberCount =
        (organizationsStore[orgIndex].activeMemberCount || 0) + 1;
    }
  ...
  async updateMember(
    ...
    if (
      request.accountStatus !== undefined &&
      request.accountStatus !== currentMember.accountStatus
    ) {
      const orgIndex = organizationsStore.findIndex(
        (org) => org.organizationId === currentMember.organizationId,
      );
      if (orgIndex !== -1) {
        const orgMembers = getMembersByOrganization(
          currentMember.organizationId,
        );
        const activeMemberCount = orgMembers.filter(
  ...
  async toggleMemberStatus(memberId: string): Promise<ApiResponse<Member>> {
    ...
    if (
      ...
      const orgMembers = getMembersByOrganization(currentMember.organizationId);
      const activeAdminCount = orgMembers.filter(
        (m) =>
          m.role === MemberRole.ADMIN &&
          m.accountStatus === AccountStatus.ACTIVE &&
          m.memberId !== memberId,
...
export const authApi = {
  ...
  async login(
    email: string,
    password: string,
  ): Promise<ApiResponse<{ member: Member; organization: Organization }>> {
    await delay(500);
    ...
    // 更新最后登录时间
    const memberIndex = membersStore.findIndex(
