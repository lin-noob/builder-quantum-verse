    ...
    const isRechartsDefaultPropsWarning = function () {
      ...
      try {
        ...
        if (args.length >= 2) {
          ...
          if (
            (firstArg.indexOf("Warning: %s: Support for defaultProps") !== -1 ||
              firstArg.indexOf("Support for defaultProps will be removed") !==
                -1) &&
        ...
        return (
          (fullMessage.indexOf("support for defaultprops") !== -1 &&
    ...
    if (typeof window !== "undefined") {
      ...
      window.onunhandledrejection = function (event) {
        if (
          event &&
          event.reason &&
          isRechartsDefaultPropsWarning(String(event.reason))
        ) {
